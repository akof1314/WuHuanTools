...
{
	// This function has some improvements thanks to sugggestions from Alexey Pismenny of Garmin Connect Sync. 2013 
	m_CtrlMap.RemoveKey(dwLocation);
...
{
	// This function has some improvements thanks to sugggestions from Alexey Pismenny of Garmin Connect Sync. 2013   
	SetItem( nItem, nSubItem, _T( "" ) );

    CCellCtrl                *pCellCtrl;

    if( m_CtrlMap.Lookup( GetMapKey( nItem, nSubItem ), pCellCtrl ) )
    {
		DeleteCellCtrl(GetMapKey( nItem, nSubItem ), pCellCtrl);
    }
}
